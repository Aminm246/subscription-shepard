...
[!NOTE] The app uses H2 in in-memory mode rather than file-based storage:
...
Jammy (Ubuntu-based) tags were chosen over Alpine-based tags for multi-architecture support — see TROUBLESHOOTING.md.
...
A dedicated, least-privilege service account (github-deployer) is created with only the roles needed to deploy:
roles/run.admin
